   ...
   */
   ...
   $(document).on('mouseover','.smr_expert', function(event) {// 부동산상담쪽 툴팁
    ...
    if($('.floating_topmenu').length > 0){
        ...
        var active_scroll_conts = ['divChildSchool','divMemulListTab','divMemulInfoTab','divCompexTab', 'divGrapPlotTab', 'divMemulListAllTab', 'divBuildTab'];
        var t_act;
        var act = 0;
        $(window).scroll(function() {
            if(floating_topmenu_live){
                if(floating_topmenu_actioncnt > 5){
                ...
                for (var i = 0; i < active_scroll_conts.length; i++) {/*스크롤시 해당화면의 메뉴 활성화*/
                    ...
                    if(obj_start < scrolltop && obj_end > scrolltop){
                        ...
                        //console.log(obj_start,obj_end,active_scroll_conts[i],i);
                    }
                    //console.log($(active_scroll_conts[i]).height());
                }
            }
        });
        $('.floating_topmenu li a').on('click',function(){/*메뉴클릭하면 스크롤 이동하는동안은 메뉴 활성화 안되게*/
